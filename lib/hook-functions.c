...
#if __arm64e__
uint64_t xpacd(uint64_t pointer) {
    asm("xpacd %[value]\n" : [value] "+r"(pointer));
    return pointer;
}

uint64_t paciza(uint64_t pointer) {
    asm("paciza %[value]\n" : [value] "+r"(pointer));
    return pointer;
}
#endif
...
int substitute_hook_functions(const struct substitute_function_hook *hooks,
    ...
    for (size_t i = 0; i < nhooks; i++) {
        struct substitute_function_hook *hook = &hooks[i];
        struct hook_internal *hi = &his[i];
        void *code = hook->function;
#if __arm64e__
        hook->replacement = (void*)xpacd((uint64_t)hook->replacement);
        code = (void*)xpacd((uint64_t)code);
#endif
        struct arch_dis_ctx arch;
...
#if __arm64e__
            *(void **) hook->old_ptr = (void *)xpacd((uint64_t)(*(void **) hook->old_ptr));
            *(void **) hook->old_ptr = (void *)paciza((uint64_t)(*(void **) hook->old_ptr));
#endif
        }
