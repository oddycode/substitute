...
#if __arm64e__
void *xpacd(void *pointer) {
    asm("xpacd %[value]\n" : [value] "+r"(pointer));
    return pointer;
}

void *paciza(void *pointer) {
    asm("paciza %[value]\n" : [value] "+r"(pointer));
    return pointer;
}
#endif
...
#if __arm64e__
        *(void**)&hook->replacement = xpacd(hook->replacement);
        code = xpacd(code);
#endif
        struct arch_dis_ctx arch;
...
#if __arm64e__
            *(void **) hook->old_ptr = xpacd(*(void **) hook->old_ptr);
            *(void **) hook->old_ptr = paciza(*(void **) hook->old_ptr);
#endif
        }
